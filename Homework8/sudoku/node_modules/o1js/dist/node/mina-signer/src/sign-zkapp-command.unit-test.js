...
import { AccountUpdate as AccountUpdateSnarky, ZkappCommand as ZkappCommandSnarky, } from '../../lib/account-update.js';
...
test(RandomTransaction.zkappCommand, (zkappCommand, assert) => {
    ...
    let ocamlCommitments = Test.hashFromJson.transactionCommitments(JSON.stringify(zkappCommandJson), 'testnet');
...
// zkapp transaction
test(RandomTransaction.zkappCommandAndFeePayerKey, RandomTransaction.networkId, (zkappCommandAndFeePayerKey, networkId) => {
    ...
    // tx commitment
    let ocamlCommitments = Test.hashFromJson.transactionCommitments(JSON.stringify(zkappCommandJson), networkId);
    let callForest = accountUpdatesToCallForest(zkappCommand.accountUpdates);
    ...
    // signature
    let sigFieldElements = signFieldElement(fullCommitment, feePayerKey, networkId);
    let sigOCaml = Test.signature.signFieldElement(ocamlCommitments.fullCommitment, Ml.fromPrivateKey(feePayerKeySnarky), networkId === 'mainnet' ? true : false);
    ...
    expect(verify(sigFieldElements, networkId === 'mainnet' ? 'testnet' : 'mainnet')).toEqual(false);
    ...
    expect(verifyZkappCommandSignature(sig, feePayerAddressBase58, networkId)).toEqual(true);
    expect(verifyZkappCommandSignature(sig, feePayerAddressBase58, networkId === 'mainnet' ? 'testnet' : 'mainnet')).toEqual(false);
});
