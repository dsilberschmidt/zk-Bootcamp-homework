...
var protocolVersions = { "txnVersion": 2 };
...
function fieldsToRustFlat([, ...fields2]) {
  ...
  for (let i2 = 0, offset = 0; i2 < n; i2++, offset += fieldSizeBytes) {
    fieldToRust(fields2[i2], flatBytes.subarray(offset, offset + fieldSizeBytes));
  }
  return flatBytes;
}
function fieldsFromRustFlat(fieldBytes) {
  ...
  for (let i2 = 0, offset = 0; i2 < n; i2++, offset += fieldSizeBytes) {
    ...
    fields2[i2] = fieldFromRust(fieldView);
  }
  return [0, ...fields2];
}
...
function createDerivers() {
  ...
  function provable3(typeObj, options) {
    ...
    function fromFields(typeObj2, fields2, aux = [], isToplevel = false) {
      ...
      if (Array.isArray(typeObj2)) {
        ...
        for (let subObj of typeObj2) {
          ...
          array.push(fromFields(subObj, fields2.slice(offset, offset + size), aux[i2]));
          offset += size;
          i2++;
        }
        return array;
    ...
    if (options?.isPure === true) {
      return {
        ...
        toAuxiliary: () => [],
        fromFields: (fields2) => fromFields(typeObj, fields2, [], true),
    ...
    return {
      ...
      fromFields: (fields2, aux) => fromFields(typeObj, fields2, aux, true),
      toInput: (obj) => toInput(obj),
      toJSON: (obj) => toJSON(obj),
      fromJSON: (json) => fromJSON(json),
      check: (obj) => check(typeObj, obj, true),
...
var Scalar3 = class _Scalar {
   ...
   */
  static fromFields(fields2) {
    return new _Scalar([0, ...fields2.map((x) => x.value)]);
  }
  /**
   * Part of the {@link Provable} interface.
   *
...
function witness(type, compute) {
  ...
  try {
    let [, ...fieldVars] = Snarky.exists(type.sizeInFields(), () => {
      ...
      let fieldConstants = fields3.map((x) => x.toConstant().value[1]);
      return [0, ...fieldConstants];
    });
    fields2 = fieldVars.map(Field4);
  } finally {
    snarkContext.leave(id2);
  }
...
function ifExplicit(condition, type, x, y) {
  ...
  let fields2 = xs.map((xi, i2) => ifField(b2, xi, ys[i2]));
...
function switch_(mask, type, values) {
  ...
  let fields2 = Array(size).fill(Field4(0));
  ...
  return {
    ...
    fromFields(fields2, aux) {
      ...
      for (let i2 = 0, offset = 0; i2 < n; i2++, offset += size) {
        array[i2] = type.fromFields(fields2.slice(offset, offset + size), aux?.[i2]);
      }
      return array;
    },
    check(array) {
...
// dist/node/lib/circuit-value.js
...
function multiRangeCheck([x, y, z]) {
  ...
  rangeCheck1Helper({ x64, x76, y64, y76, z, yz: zero3 });
}
...
var CircuitValue = class {
  ...
  static toFields(v) {
    ...
    for (let i2 = 0, n = fields2.length; i2 < n; ++i2) {
      const [key, propType] = fields2[i2];
...
var ProvableField = {
  ...
  toInput: (x) => ({ fields: [x] }),
  toJSON: Field3.toJSON,
  fromJSON: Field3.fromJSON,
  empty: () => zero2
};
...
function fields(length) {
  return {
    ...
    toInput: (x) => ({ fields: x }),
    toJSON: (x) => x.map(Field3.toJSON),
    fromJSON: (x) => x.map(Field3.fromJSON),
    empty: () => new Array(length).fill(zero2)
  };
}
...
function buildXor(a2, b2, out, padLength) {
  ...
  let zero3 = new Field3(0);
  zero3.assertEquals(a2);
  zero3.assertEquals(b2);
  zero3.assertEquals(out);
}
...
// dist/node/lib/provable-types/packed.js
var Packed = class _Packed {
  ...
  unpack() {
    ...
    let packed = packToFields(input);
    for (let i2 = 0; i2 < this.packed.length; i2++) {
      this.packed[i2].assertEquals(packed[i2]);
    }
    return value;
  }
  toFields() {
...
function packToFieldsLegacy({ fields: fields2, bits: bits2 }) {
  let packedFields = [];
  while (bits2.length > 0) {
...
function deriveNonce(message, publicKey, privateKey, networkId) {
  ...
  let id2 = networkId === "mainnet" ? networkIdMainnet : networkIdTestnet;
  ...
  function fromFields(typeData, fields2, aux) {
    ...
    if (typeData.type === "array") {
      ...
      for (let i2 = 0, offset = 0; i2 < length; i2++, offset += size) {
        value[i2] = fromFields(arrayTypeData.inner, fields2.slice(offset, offset + size), aux[i2]);
      }
      return value;
    }
    if (typeData.type === "option") {
    ...
    if (typeData.type === "object") {
      ...
      for (let i2 = 0; i2 < keys.length; i2++) {
        ...
        values[keys[i2]] = fromFields(typeEntry, fields2.slice(offset, offset + size), aux[i2]);
        offset += size;
      }
      return values;
    }
...
var MerkleList = class _MerkleList {
  ...
  push(element) {
    ...
    this.data.updateAsProver((data) => [
      toConstant3(this.innerProvable, { previousHash, element }),
      ...data
    ]);
  }
  /**
  ...
  pushIf(condition, element) {
    ...
    this.data.updateAsProver((data) => condition.toBoolean() ? [toConstant3(this.innerProvable, { previousHash, element }), ...data] : data);
  }
  popWitness() {
  ...
  popIf(condition) {
    ...
    this.data.updateAsProver((data) => {
      ...
      return condition.toBoolean() ? data : [toConstant3(this.innerProvable, node), ...data];
    });
  ...
  static create(type, nextHash = merkleListHash(type), emptyHash_ = emptyHash) {
    class MerkleListTBase extends _MerkleList {
      ...
      static from(array) {
        ...
        let unconstrained = Unconstrained.witness(() => data.map((x) => toConstant3(type, x)));
...
var MerkleListIterator = class _MerkleListIterator {
  ...
  next() {
    ...
    this.currentIndex.updateAsProver((i2) => Math.min(i2 + 1, this.data.get().length));
...
function withHashes(data, nextHash, emptyHash2) {
  ...
  for (let i2 = n - 1; i2 >= 0; i2--) {
    arrayWithHashes[i2] = { previousHash: currentHash, element: data[i2] };
    currentHash = nextHash(currentHash, data[i2]);
  }
...
function assertEquals(property, value) {
...
function hash2(memo) {
  ...
  let fields2 = packToFieldsLegacy(HashInputLegacy.bits(bits2));
...
// dist/node/lib/account-update.js
...
var Permissions = {
  ...
  default: () => ({
    ...
    setVerificationKey: {
      ...
      txnVersion: TransactionVersion3.current()
    },
...
var AccountUpdate3 = class _AccountUpdate {
  ...
  token() {
    ...
    let id2 = TokenId4.derive(tokenOwner, parentTokenId);
    function getApprovedAccountUpdate(accountLike, label) {
      ...
      if (accountLike instanceof _AccountUpdate) {
        accountLike.tokenId.assertEquals(id2);
        thisAccountUpdate.approve(accountLike);
      }
      if (accountLike instanceof PublicKey2) {
        accountLike = _AccountUpdate.defaultAccountUpdate(accountLike, id2);
        thisAccountUpdate.approve(accountLike);
      }
  ...
  hash() {
    if (Provable.inCheckedComputation()) {
      ...
      return hashWithPrefix(activeInstance.getNetworkId() === "mainnet" ? prefixes.zkappBodyMainnet : prefixes.zkappBodyTestnet, packToFields(input));
    } else {
      let json = transaction_exports.AccountUpdate.toJSON(this);
      return Field4(Test.hashFromJson.accountUpdate(JSON.stringify(json), activeInstance.getNetworkId()));
    }
  }
  toPublicInput({ accountUpdates }) {
    ...
    let i2 = accountUpdates.findIndex((a2) => a2.id === this.id);
    assert(i2 !== -1, "Account update not found in transaction");
    for (i2++; i2 < accountUpdates.length; i2++) {
      let update = accountUpdates[i2];
...
function hashAccountUpdate(update) {
  return genericHash(AccountUpdate3, activeInstance.getNetworkId() === "mainnet" ? prefixes.zkappBodyMainnet : prefixes.zkappBodyTestnet, update);
}
...
var AccountUpdateForest = class _AccountUpdateForest extends MerkleList.create(AccountUpdateTreeBase, merkleListHash2) {
  ...
  static fromSimpleForest(simpleForest) {
    ...
    return _AccountUpdateForest.from(nodes);
  }
...
function addMissingSignatures(zkappCommand, additionalKeys = []) {
  ...
  let { commitment, fullCommitment } = transactionCommitments(transaction_bigint_exports.ZkappCommand.fromJSON(ZkappCommand3.toJSON(zkappCommand)), activeInstance.getNetworkId());
...
function LocalBlockchain({ proofsEnabled = true, enforceTransactionLimits = true, networkId = "testnet" } = {}) {
  ...
  return {
    ...
    async sendTransaction(txn) {
      ...
      let commitments = transactionCommitments(transaction_bigint_exports.ZkappCommand.fromJSON(zkappCommandJson), minaNetworkId);
...
function bytesFromFields(fields2) {
  ...
  let bytes = new Uint8Array(fields2.length * 31 + i2);
  bytes.set(lastChunkBytes.subarray(0, i2), fields2.length * 31);
  i2 = 0;
...
Note: Only types built out of \`Field\` are valid return types. This includes o1js primitive types and custom CircuitValues.`;
      ...
      let { accountUpdate, result: { result, children } } = AccountUpdate3.witness(provable({
    ...
    if (!methodIntfs.every((m) => m.methodName in methodMetadata) && !inAnalyze()) {
      ...
      try {
        for (let methodIntf of methodIntfs) {
          let accountUpdate;
          let { rows, digest, result, gates, summary } = analyzeMethod(ZkappPublicInput, methodIntf, (publicInput, publicKey, tokenId, ...args) => {
...
function diffRecursive(prover, inputData) {
  ...
  for (let i2 = 0; i2 < nChildren; i2++) {
    let inputChild = inputChildren[i2].mutable;
    let child = proverChildren[i2].mutable;
    if (!inputChild || !child)
      return;
...
var TokenAccountUpdateIterator = class _TokenAccountUpdateIterator {
  ...
  static create(forest, selfToken) {
    return new _TokenAccountUpdateIterator(AccountUpdateIterator.startIterating(forest), MayUseToken.ParentsOwnToken, selfToken);
  }
  /**
  ...
  assertFinished(message) {
    assert3(this.currentLayer.forest.isAtEnd(), message ?? "TokenAccountUpdateIterator not finished");
  }
};
...
var TokenContract = class extends SmartContract {
  ...
  forEachUpdate(updates, callback) {
    ...
    for (let i2 = 0; i2 < MAX_ACCOUNT_UPDATES; i2++) {
