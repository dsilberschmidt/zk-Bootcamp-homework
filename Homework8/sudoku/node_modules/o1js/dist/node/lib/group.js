...
class Group {
    ...
    add(g) {
        if (isConstant(this) && isConstant(g)) {
            ...
            let [, x, y] = Snarky.gates.ecAdd(toTuple(Group.from(x1.seal(), y1.seal())), toTuple(Group.from(x2.seal(), y2.seal())), toTuple(Group.from(x3, y3)), inf.toField().value, same_x.value, s.value, inf_z.value, x21_inv.value);
