...
function negateIf(condition, P, f) {
    let y = Provable.if(Bool.Unsafe.ofField(condition), Field3.provable, ForeignField.negate(P.y, f), P.y);
    return { x: P.x, y };
}
...
function decomposeNoRangeCheck(Curve, s) {
    ...
    let lambda = Provable.if(Bool.Unsafe.ofField(s1Negative), Field3.provable, Field3.from(Curve.Scalar.negate(Curve.Endo.scalar)), Field3.from(Curve.Endo.scalar));
    let rhs = Provable.if(Bool.Unsafe.ofField(s0Negative), Field3.provable, ForeignField.Sum(s).add(s0).finish(Curve.order), ForeignField.Sum(s).sub(s0).finish(Curve.order));
