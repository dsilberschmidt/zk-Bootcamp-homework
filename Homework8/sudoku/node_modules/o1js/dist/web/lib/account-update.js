import { cloneCircuitValue, provable, provablePure, StructNoJson, } from './circuit-value.js';
...
import { dummyBase64Proof, Prover } from './proof-system.js';
...
import { emptyHash, genericHash, MerkleList, MerkleListBase, } from './provable-types/merkle-list.js';
import { Hashed } from './provable-types/packed.js';
import { accountUpdateLayout, smartContractContext, } from './mina/smart-contract-context.js';
...
// internal API
export { Permission, Preconditions, Body, Authorization, ZkappCommand, addMissingSignatures, addMissingProofs, Events, Actions, TokenId, Token, zkAppProver, dummySignature, AccountUpdateTreeBase, AccountUpdateLayout, hashAccountUpdate, HashedAccountUpdate, };
...
class AccountUpdate {
    ...
    hash() {
        ...
        if (Provable.inCheckedComputation()) {
            ...
            return hashWithPrefix(activeInstance.getNetworkId() === 'mainnet'
                ? prefixes.zkappBodyMainnet
                : prefixes.zkappBodyTestnet, packToFields(input));
        }
        else {
            let json = Types.AccountUpdate.toJSON(this);
            return Field(Test.hashFromJson.accountUpdate(JSON.stringify(json), activeInstance.getNetworkId()));
        }
    }
...
function hashAccountUpdate(update) {
    return genericHash(AccountUpdate, activeInstance.getNetworkId() === 'mainnet'
        ? prefixes.zkappBodyMainnet
        : prefixes.zkappBodyTestnet, update);
}
...
function addMissingSignatures(zkappCommand, additionalKeys = []) {
    ...
    let { commitment, fullCommitment } = transactionCommitments(TypesBigint.ZkappCommand.fromJSON(ZkappCommand.toJSON(zkappCommand)), activeInstance.getNetworkId());
