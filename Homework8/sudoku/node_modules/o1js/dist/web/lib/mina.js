...
import { addMissingProofs, addMissingSignatures, ZkappCommand, AccountUpdate, ZkappPublicInput, TokenId, Authorization, Actions, dummySignature, AccountUpdateLayout, } from './account-update.js';
import * as Fetch from './fetch.js';
import { assertPreconditionInvariants } from './precondition.js';
import { cloneCircuitValue } from './circuit-value.js';
import { verify } from './proof-system.js';
...
function LocalBlockchain({ proofsEnabled = true, enforceTransactionLimits = true, networkId = 'testnet', } = {}) {
    ...
    return {
        ...
        async sendTransaction(txn) {
            ...
            let commitments = transactionCommitments(TypesBigint.ZkappCommand.fromJSON(zkappCommandJson), minaNetworkId);
...
async function verifyAccountUpdate(account, accountUpdate, publicInput, transactionCommitments, proofsEnabled, networkId) {
    ...
    if (accountUpdate.authorization.signature) {
        ...
        try {
            isValidSignature = verifyAccountUpdateSignature(TypesBigint.AccountUpdate.fromJSON(accountUpdateJson), transactionCommitments, networkId);
        }
        catch (error) {
            errorTrace += '\n\n' + error.stack;
